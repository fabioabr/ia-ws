...
REPORT_SETUP_TO_DELIVERABLES_SCOPE = {
    "essential": ["DR", "OP"],
    "executive": ["DR", "OP", "EX"],
    "complete": ["DR", "OP", "EX"],
}
...
def generate_config_md(
    ...
    *,
    ...
    scope_csv = ", ".join(deliverables_scope)
    scoring_label = {
        "padrão": "padrão (>=90%)",
...
deliverables-scope: [{scope_csv}]
report-setup: {report_setup}  # legacy alias — canonical is deliverables-scope
...
| Context-Templates | {templates_csv} |
| Deliverables Scope | {scope_csv} |
| Report Setup (legacy) | {report_setup} |
...
def create_run(
    ...
    # Canonical: deliverables_scope / deliverables-scope (list of "DR","OP","EX")
    # Legacy alias: report-setup (essential|executive|complete) — still read for backward compat
    raw_scope = fm.get("deliverables_scope", fm.get("deliverables-scope"))
    report_setup: str = fm.get("report-setup", "complete")
    if raw_scope is None:
        # Fallback: derive from legacy report-setup
        deliverables_scope: list[str] = REPORT_SETUP_TO_DELIVERABLES_SCOPE.get(
            report_setup, ["DR", "OP", "EX"]
        )
        print(
            f"  NOTE: briefing has no deliverables_scope; derived {deliverables_scope} "
            f"from legacy report-setup={report_setup!r}. Prefer declaring deliverables_scope directly."
        )
    elif isinstance(raw_scope, list):
        deliverables_scope = [str(x).strip().upper() for x in raw_scope]
    elif isinstance(raw_scope, str):
        # Support "[DR, OP]" or "DR, OP"
        cleaned = raw_scope.strip().strip("[]")
        deliverables_scope = [t.strip().strip("\"'").upper() for t in cleaned.split(",") if t.strip()]
    else:
    ...
    print(f"  Deliverables scope: {', '.join(deliverables_scope)}")
    print(f"  Report setup (legacy): {report_setup}")
