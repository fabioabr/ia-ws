...
| # | Region | Layout | O que mostra |
|---|--------|--------|-------------|
| 57 | REG-RISK-02 | full-width | Riscos técnicos (tabela com severity) |
| 58 | REG-RISK-04 | full-width | Análise de viabilidade (radar 4 dimensões — Chart.js) |
| 59 | REG-QUAL-03 | full-width | Gaps identificados |
| 60 | REG-QUAL-04 | full-width | Checklist de conclusão (progress bar) |

#### Seção I — Backlog Detalhado

| # | Region | Layout | O que mostra |
|---|--------|--------|-------------|
| 61 | REG-BACK-02 | full-width | User stories de alto nível (accordion) |
| 62 | REG-BACK-03 | full-width | Dependências entre épicos |
| 63 | REG-BACK-04 | full-width | Critérios de Go/No-Go (traffic light) |

#### Seção J — Métricas Completas

| # | Region | Layout | O que mostra |
|---|--------|--------|-------------|
| 64 | REG-METR-01 | grid-3 | KPIs de negócio (stat cards) |
| 65 | REG-METR-02 | grid-3 | KPIs técnicos (stat cards) |
| 66 | REG-METR-03 | full-width | SLAs e SLOs (tabela com gauges) |
| 67 | REG-METR-04 | full-width | Targets por fase (timeline) |
| 68 | REG-METR-05 | grid-4 | DORA metrics (4 stat cards — quando platform) |

#### Seção K — Narrativa Completa

| # | Region | Layout | O que mostra |
|---|--------|--------|-------------|
| 69 | REG-NARR-02 | full-width | Condições para prosseguir (checklist) |
| 70 | REG-NARR-03 | full-width | Assinaturas de aprovação (tabela formal) |

#### Seção L — Domain-Specific (condicional por context-template)

| # | Region | Layout | Quando |
|---|--------|--------|--------|
| 71 | REG-DOM-SAAS-01 | full-width | `saas` — Pricing model |
| 72 | REG-DOM-SAAS-02 | full-width | `saas` — Tenancy strategy |
| 73 | REG-DOM-DATA-01 | full-width | `datalake-ingestion` — Medallion |
| 74 | REG-DOM-DATA-02 | full-width | `datalake-ingestion` — Data quality |
| 75 | REG-DOM-INTEG-01 | full-width | `system-integration` — Mapa de integrações |
| 76 | REG-DOM-INTEG-02 | full-width | `system-integration` — Data contracts |
| 77 | REG-DOM-MIGR-01 | full-width | `migration-modernization` — Roadmap |
| 78 | REG-DOM-MIGR-02 | full-width | `migration-modernization` — AS-IS vs TO-BE |
| 79 | REG-DOM-AIML-01 | full-width | `ai-ml` — ML strategy |
| 80 | REG-DOM-AIML-02 | full-width | `ai-ml` — Model governance |
| 81 | REG-DOM-MOB-01 | full-width | `mobile-app` — Mobile strategy |
| 82 | REG-DOM-MOB-02 | full-width | `mobile-app` — Distribution |
| 83 | REG-DOM-RPA-01 | full-width | `process-automation` — Automation roadmap |
| 84 | REG-DOM-RPA-02 | full-width | `process-automation` — CoE governance |
| 85 | REG-DOM-PLAT-01 | full-width | `platform-engineering` — Platform architecture |
| 86 | REG-DOM-PLAT-02 | grid-4 | `platform-engineering` — DX metrics |
| 87 | REG-DOM-PROC-01 | full-width | `process-documentation` — Taxonomy |
| 88 | REG-DOM-PROC-02 | full-width | `process-documentation` — Governance |
| 89 | REG-DOM-MICRO-01 | full-width | `web-microservices` — Service map |
| 90 | REG-DOM-MICRO-02 | full-width | `web-microservices` — Resilience |

**Total: até 90 regions** (70 universais + até 20 domain-specific dependendo do context-template)
...
| `full-report.html` | 70-90 | Sim (radar, bubble, line, donut, stacked bar) | Time técnico, arquiteto, PO |
