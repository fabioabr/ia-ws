...
└── report-setups/            ← **legacy** — catálogo de regions por HTML (consumido pelo html-writer)
    ├── essential.md          ← regions do one-pager.html (equiv. `deliverables_scope: ["DR","OP"]`)
    ├── executive.md          ← regions do executive-report.html (equiv. `["DR","OP","EX"]`)
    └── complete.md           ← regions do full-report.html (equiv. `["DR","OP","EX"]`)
```
