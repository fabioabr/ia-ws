...
| # | Tópico | Pergunta-chave | Condicional |
|---|---|---|---|
| 1 | **Objetivo estratégico** | Qual o objetivo de negócio? | sempre |
| 2 | **Key Results mensuráveis** | Métricas específicas com meta e prazo | sempre |
| 3 | **ROI esperado** | Em quanto tempo paga o investimento? | **apenas se `require_roi=true` no config.md** |
| 4 | **Métricas de adoção** | Como saber se está sendo usado? | sempre |
| 5 | **Métricas de qualidade** | Como saber se está entregando bem? | sempre |
| 6 | **Classificação de requisitos** | Mandatórios / desejáveis / opcionais | sempre |
| 7 | **MVP + fases futuras** | Horizontes de entrega | sempre |
...
> [!warning] ROI condicional
> O tópico 3 (ROI esperado) só é cobrado quando `require_roi: true` no `config.md` do run. O default é `false` — muitas organizações aprovam o investimento **antes** do discovery (comitê → liberação → discovery), tornando a cobrança de ROI no entregável do discovery redundante. Quando pulado, a seção 3.3 do `product-vision.md` é **omitida** (não aparece como "N/A"). Ver [[rules/discovery/discovery]] seção "Flags de configuração".
...
### 3.3 ROI esperado                          <!-- OMITIR se require_roi=false -->
