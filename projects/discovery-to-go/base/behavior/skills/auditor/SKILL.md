...
13. `{project}/setup/briefing.md` — **ler flags** (`financial_model`, `require_roi`, `deliverables_scope`) — definem quais critérios são aplicáveis nesta run
14. `{project}/setup/customization/rules/scoring-thresholds.md` (ou fallback `base/starter-kit/client-template/templates/customization/scoring-thresholds.md`) — pisos e pesos customizáveis

### 2. Modo de operação

Você opera em **1 modo**: medir e pontuar. Não há variação.

> [!warning] Critérios condicionais às flags do briefing
> O auditor **não pode penalizar** a ausência de elementos que foram explicitamente declarados opcionais pelas flags do briefing:
>
> - Se `require_roi=false`: **não exigir** ROI, payback, break-even em `1.3-value-and-okrs.md`. Cobertura desses tópicos é neutra (nem soma nem subtrai).
> - Se `financial_model=fundo-global`: **não exigir** TCO completo em `1.8-tco-and-build-vs-buy.md`; exigir em seu lugar **"estimativa de consumo cloud sem free tier"** (escopo e profundidade equivalentes).
> - A **validação automática de viabilidade financeira** (Receita × TCO) é executada **apenas quando** `financial_model=projeto-paga` AND `require_roi=true`. Caso contrário, seção é omitida do relatório com nota "não aplicável (flags do briefing)".
>
> O checklist de Completude (Dimensão 1) **deve ser filtrado** pelas flags antes de calcular `N` — remover tópicos marcados como condicionais cuja condição falhou.
...
> [!info] Condicional às flags
> Esta validação **só é executada** quando `financial_model=projeto-paga` AND `require_roi=true`. Em outros cenários (fundo-global, aprovação upstream), comparar Receita × TCO não faz sentido — o relatório deve apenas registrar "N/A (flags do briefing: financial_model={valor}, require_roi={valor})".
...
#### Validação alternativa: overrun de consumo (modo fundo-global)

Quando `financial_model=fundo-global`, a validação acima **não se aplica**. Em seu lugar, o auditor verifica se o bloco 1.8 entregou **estimativa de consumo cloud sem free tier** com:

- Volumes por serviço (GB, req/mês, horas de CPU, etc.)
- Custo estimado por ano (Ano 1, 2, 3)
- Premissas explicitadas
- Alertas de overrun definidos

**Penalidade:** ausência de quaisquer dos itens acima penaliza em -10 pontos na dimensão "Completude".
