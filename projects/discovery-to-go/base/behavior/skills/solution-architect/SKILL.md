...
O comportamento desta área depende do flag `financial_model` declarado no `config.md` do run:

- `financial_model=projeto-paga` (default) → análise econômica convencional (tabela abaixo)
- `financial_model=fundo-global` → modo "estimativa de consumo" (ver bloco específico abaixo)

##### Modo `projeto-paga` (padrão)
...
Organizações que operam com fundo corporativo centralizado de OPEX cloud (o projeto **não arca** o custo — consome do fundo). O objetivo da análise deixa de ser "custo total do projeto" e passa a ser "estimativa de consumo mensal **sem considerar free tier**" para o planejamento do fundo.

| # | Tópico | Pergunta-chave |
|---|---|---|
| 1 | **Alternativas Buy** | Quais 2-3 SaaS/produtos avaliar? (ainda obrigatório) |
| 2 | **Alternativa Build** | Construir custom é viável? |
| 3 | **Critérios (sem custo)** | Mandatórios, fronteiras técnicas, **sustentabilidade**, **vendor lock-in**, time to value |
| 4 | **Estimativa de consumo cloud** | Mensal **sem free tier** — por serviço (compute, storage, egress, managed services) |
| 5 | **Projeção de crescimento** | Como o consumo escala com adoção/uso? |
| 6 | **Licenças de terceiros** | Ainda relevantes (não vêm do fundo global) |
| 7 | **Recomendação formal** | Build / Buy / Híbrido com justificativa **baseada em sustentabilidade**, não custo |
| 8 | **Premissas do fundo global** | Quais serviços cloud são pré-aprovados pela governança? |

> [!warning] Fundo global ≠ "não importa o custo"
> O fundo é finito. A estimativa de consumo sem free tier alimenta o planejamento orçamentário do fundo no horizonte do projeto. Subdimensionar é tão grave quanto inflar TCO em modo `projeto-paga`.
...
### TCO obrigatório (condicional ao `financial_model`)

#### Quando `financial_model=projeto-paga`
...
#### Quando `financial_model=fundo-global`

Substitui TCO convencional por **Estimativa de Consumo Cloud Mensal**:

| Categoria | Detalhe |
|---|---|
| **Consumo compute** | vCPU/RAM/h mensais, **sem free tier**, por serviço (app, worker, db, cache) |
| **Consumo storage** | GB-mês por tipo (hot, cool, archive); growth rate |
| **Consumo egress** | GB/mês out-of-region e out-to-internet |
| **Consumo managed services** | Requests, conexões, transações por serviço PaaS usado |
| **Licenças externas** | Software de terceiros que NÃO vêm do fundo global (continuam como TCO) |

**Saída da Estimativa:**
- Consumo mensal estimado (R$ ou USD) no MVP e em regime
- Breakdown por serviço cloud
- Premissas (provider, região, SKU, tier)
- Projeção de crescimento (consumo vs. adoção)
- Alertas de outliers (serviços caros > X% do total)
...
## 2. TCO 3 anos                               <!-- apenas se financial_model=projeto-paga -->
### 2.1 Breakdown por categoria
### 2.2 Breakdown anual (Y1, Y2, Y3)
### 2.3 Premissas explícitas
### 2.4 Sensibilidade

## 2. Estimativa de Consumo Cloud               <!-- apenas se financial_model=fundo-global -->
### 2.1 Consumo mensal por serviço (sem free tier)
### 2.2 Projeção de crescimento
### 2.3 Premissas (provider, região, SKU, tier)
### 2.4 Licenças externas fora do fundo global
...
- TCO 3 anos é obrigatório **quando `financial_model=projeto-paga`**. Com premissas explícitas e sensibilidade.
- Estimativa de Consumo Cloud mensal é obrigatória **quando `financial_model=fundo-global`**. Sem free tier, com premissas de provider/região/SKU.
