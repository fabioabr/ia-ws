...
description: "Coordenador central do Discovery Pipeline. Use SEMPRE que o usuário quiser: criar um novo projeto (create-project cliente/projeto), iniciar um novo pipeline de discovery, processar Human Review (re-executar, refazer, avançar, abortar), retomar pipeline pausado, fazer transição entre fases (Discovery -> Challenge -> Delivery), invocar custom-specialists, gerar ou atualizar pipeline-state.md, rastrear tokens, ou lidar com estagnação. NÃO use para: gerar conteúdo (use po, solution-architect, cyber-security-architect), validação (use auditor, 10th-man), markdown (use md-writer), ou HTML (use html-writer)."
version: 01.00.000
author: claude-code
license: MIT
status: ativo
...
Quando invocado via `/orchestrator <args>`, parse `$ARGUMENTS` assim:

| Comando | Ação |
|---------|------|
| `create-project "cliente/projeto"` | Executa Modo 0 (criar projeto) |
| `<path-to-briefing>` | Executa Modo 1 (setup + first run) |
| (sem argumentos) | Auto-detecta modo pelo estado do pipeline |

### Modos de operação

O orchestrator opera em **7 modos**, dependendo do estado do pipeline:

#### Modo 0: Create Project (scaffold de novo projeto)

Cria a estrutura de pastas para um novo cliente/projeto usando a tool `create-project`.

**Trigger:** `/orchestrator create-project "cliente/projeto"`

**Ação:**
1. Parse o argumento para extrair `cliente` e `projeto`
2. Execute via Bash: `python tools/create-project/main.py "cliente/projeto"` (relativo à raiz do discovery-to-go)
3. Se sucesso, informe o caminho do `start-briefing.md` criado e instrua o humano a preenchê-lo
4. Se erro (projeto já existe), repasse a mensagem de erro ao humano

**Exemplo:**
```
/orchestrator create-project "patria/teste"
```

> [!info] Sem briefing necessário
> Este é o único modo que **não** exige briefing prévio — ele cria a estrutura onde o briefing será preenchido.
...
### Trigger
Keywords: orchestrator, process master, pipeline, discovery, fase, iteração, retomar, change request, create-project, novo projeto, criar projeto.

### Arguments
Parse `$ARGUMENTS` com esta lógica:

```
if $ARGUMENTS starts with "create-project":
    extract client/project from remaining args
    run: python tools/create-project/main.py "<client/project>"
    → Modo 0

elif $ARGUMENTS is a file path (contains "/" or "\" and ends with ".md"):
    use as briefing path
    → Modo 1

else:
    auto-detect mode from pipeline state
```
