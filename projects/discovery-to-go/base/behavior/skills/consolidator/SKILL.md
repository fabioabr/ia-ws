...
#### Adaptação de tom por entregável (`deliverables_scope`)

> **Nota (task #18):** esta seção foi reescrita para `deliverables_scope`. A versão anterior usava `report-setup: essential|executive|complete` — ainda aceito como alias legacy, mas o canônico é `deliverables_scope` (ADR-001). O tom do **DR** é sempre técnico; os tons comercial/executivo aparecem no OP e no EX **destilados do DR pelo `deliverable-distiller`**. Você escreve o DR com toda a profundidade — o distiller ajusta a linguagem por público.

| Entregável em `deliverables_scope` | Onde o tom é aplicado | Público | Linguagem |
|-----------|------------------------|---------|-----------|
| `DR` (sempre presente) | Este arquivo (seu output) | Arquiteto, PO, dev | Técnica completa, stack, APIs, protocolos, patterns — sem simplificar |
| `OP` (destilado) | `one-pager.md` gerado pelo distiller | Sponsor, comercial, C-level | Horas, semanas, escopo. Sem jargão técnico. |
| `EX` (destilado) | `executive-report.md` gerado pelo distiller | Diretoria, gestão | Custos, prazos, riscos, modelo de negócio. Termos técnicos explicados. |

**Para regions domain-specific:** escreva no DR a versão técnica completa. O distiller adapta para OP/EX usando os layouts (`one-pager-layout.md`, `executive-layout.md`) — você **não** precisa decidir quais vão para cada entregável. Exemplo: `REG-DOM-SAAS-01` (pricing) entra em qualquer layout; `REG-DOM-SAAS-02` (tenancy/isolamento técnico) tende a ficar só no DR.
