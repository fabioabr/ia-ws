...
3. **`{project}/setup/customization/html-layout.md`** — se existir, define a ordem e disposição das regions. Se não existir, usar o default em `starter-kit/client-template/templates/customization/html-layout.md`.
4. **Catálogo de regions** em `standards/conventions/report-regions/schemas/README.md` — para consultar schema, tipo visual e chart-specialist recommendation de cada region.
5. **Previews HTML** em `standards/conventions/report-regions/samples/` — 15 arquivos HTML que mostram visualmente como cada grupo de regions é renderizado. Consulte como referência visual para suas decisões.
