...
| Componentes | `conventions/components/*.md` | Card, alerts, badge, table, stat-card, header-footer, tabs, **gantt-hierarchical**, **section-collapsible** |
...
2. `starter-kit/client-template/templates/customization/html-layout.md` (default)
3. Se nenhum existir: renderizar todas as regions na ordem do `.md` como full-width

#### Templates visuais por region

Cada region tem uma recomendação de visualização no catálogo (`standards/conventions/report-regions/schemas/{grupo}/{region}.md` → seção "Recomendação do Chart Specialist"). Aplicar:
...
HTMLs de referência com exemplos visuais de cada grupo estão em `standards/conventions/report-regions/samples/`. Consultar para entender como cada region deve ser renderizada.
...
**Processo:**
1. Consultar `standards/conventions/writing/acronyms/acronym-bank.md` para expansões
...
### P48 — Gantt hierárquico (épico → estória) com linha-guia de marco

Quando o Gantt representar um cronograma com marco contratual (ex: Release do Quick Win), aplicar o padrão `gantt-hierarchical` (ver `conventions/components/gantt-hierarchical.md`):

1. **1 coluna por semana** (`<th>1</th>...<th>24</th>`) — nunca agrupar 2 semanas por coluna
2. **Linha de marco é a primeira do `<tbody>`** — ancora o leitor no marco antes dos épicos
3. **Linha do marco SEM `border-left`** no primeiro td — apenas fundo verde uniforme em todas as células
4. **Estrela ⭐** (`<span class="gantt-star">⭐</span>`) no cruzamento marco × semana-alvo E na estória-chave que representa a entrega — **nunca no épico**
5. **`tr.gantt-epic td.week`** (especificidade 0,2,2) para escurecer todas as 24 células da linha de épico, inclusive as vazias
6. **Intersecção épico × coluna QW** usa técnica layered: `background-color: rgba(0,0,0,0.14); background-image: linear-gradient(var(--success-light), var(--success-light));`
7. **`vertical-align: middle`** em `.gantt-bar` (obrigatório para alinhar barras que compartilham célula com emoji)
8. **`position: relative`** em `.gantt-table td.week` — ancora a estrela absoluta
9. **Cabeçalho da coluna QW** tem `::after` com `⚡` no canto

### P49 — Toda `.section` do relatório é recolhível

Toda `<div class="section">` DEVE ter um botão de expand/collapse injetado dinamicamente via JS (ver `conventions/components/section-collapsible.md`):

- `.section-header` ganha `cursor: pointer; user-select: none` e listener de click
- Botão `.section-toggle` é criado com ícone `ri-arrow-down-s-line` e inserido com `margin-left: auto`
- Estado `.section.collapsed` oculta todos os filhos exceto o header e gira o chevron -90°
- Borda do header vira tracejada (`1px dashed`) quando recolhido
- O JS é adicionado no final do `<script>` junto com o tab-switcher e o theme-toggle
...
9. **Gantt hierárquico** (quando houver marco): linha de marco é primeira do tbody, estrela na estória-chave (P48)
10. **Toda `.section`** tem botão de expand/collapse injetado via JS (P49)
...
- Gantt hierárquico: marco é primeira linha, estrela na estória (não no épico), 1 col/semana (P48)
- Toda `.section` é recolhível via botão injetado (P49)
...
|--------|------|-----------|
| 02.07.000 | 2026-04-17 | P48: Gantt hierárquico épico→estória com linha de marco primeira, estrela na estória-chave (não no épico), 1 coluna por semana, intersecção layered épico×coluna QW. P49: toda `.section` é recolhível via botão injetado por JS. Componentes `gantt-hierarchical.md` e `section-collapsible.md` adicionados. |
