---
title: "Priorização de Perguntas por Entregável — Template"
description: "Template que mapeia cada pergunta do environment.md para os entregáveis One-Pager / Executive / Delivery usando bitmap [OP][EX][DR]. Copie para o projeto e adapte com o estado do cliente."
category: framework-template
type: question-priority-template
status: default
version: "01.00.000"
origin: "base/standards/blueprints/environment/question-priority-template.md"
related:
  - "base/standards/blueprints/environment/README.md"
  - "base/behavior/skills/deliverable-distiller/SKILL.md"
  - "base/behavior/rules/discovery/discovery.md"
---

# Priorização de Perguntas por Entregável — Template

> Este template responde: **"Para gerar o One-Pager, preciso responder tudo?"** Não. Cada entregável tem um subconjunto mínimo de perguntas do `environment.md` que precisa estar respondido para ser útil. Este arquivo mapeia quais perguntas alimentam quais entregáveis.

## Como usar

1. Copie este arquivo para `projects/{cliente}/kb/question-priority.md`.
2. Complete o frontmatter com `company: "<Nome>"` e `last-updated`.
3. Percorra os domínios e ajuste o bitmap conforme o contexto do cliente (contratos, modelo financeiro, compliance aplicável).
4. Marque o checklist final com `[X]` conforme as respostas forem sendo coletadas do cliente.
5. Este arquivo é **input para o skill `deliverable-distiller`** (Fase 3 do pipeline) — mas como **sugestão**, não como filtro mecânico (ver [!info] abaixo).

> [!info]
> **ADR-001 — o bitmap é sugestão, não filtro.** O `deliverable-distiller` recebe o layout (`one-pager-layout.md` ou `executive-layout.md`) como **lei** e este bitmap como **hint** para decidir ambiguidades (quando uma region obrigatória pode ser resumida ou expandida). O distiller nunca remove region obrigatória só porque o bitmap marcou `[ ]`, e nunca inclui region não-listada no layout só porque o bitmap marcou `[X]`.
...
1. **One-Pager foca em ESFORÇO, CUSTO e ESCOPO.** Responde: *o que vai ser feito, quanto custa (estimativa), quanto esforço/duração*. Top 1-2 riscos e premissas-chave.
   - **NÃO entra no One-Pager por padrão:** FTE/disponibilidade real do time (assumir "todos contratados"); ROI/payback (só se `require_roi=true` no briefing).
...
### Ajustes condicionais às flags do briefing

O bitmap é um **default conservador**. Os ajustes abaixo são aplicados implicitamente pelo `deliverable-distiller` conforme as flags do briefing (ver [discovery.md](../../../behavior/rules/discovery/discovery.md) — seção "Flags de configuração"):

| Flag do briefing | Ajuste no bitmap |
|---|---|
| `require_roi=false` (default) | ROI/payback/break-even são `[ ][ ][X]` mesmo que o template genérico sugira outro valor |
| `financial_model=fundo-global` | Budget por projeto vira `[ ][ ][ ]`; Estimativa de consumo cloud vira `[X][X][X]` |
| `deliverables_scope=["DR"]` | Bitmap é informativo apenas (não há distillation); o DR cobre tudo |

## Resumo agregado

| Entregável | Regions alimentadas | Perguntas mínimas do `environment.md` | Status atual |
|-----------|---------------------|--------------------------------------|--------------|
| **One-Pager** | executive (overview-one-pager, go-no-go-decision, next-steps, premises, product-brief) | **~8 perguntas** (escopo/custo/esforço) | _A preencher_ |
| **Executive** | + product, financial, risk, organization, security (alto nível), privacy (alto nível) | **~120 perguntas** | _A preencher_ |
| **Delivery** | + technical, domain, metrics, narrative, research, backlog, quality, security (detalhe), privacy (detalhe) | **~todas (564 no blueprint-base)** | _A preencher_ |

## Mapeamento regions → domínios do environment.md

Cada region do delivery report consome respostas de um ou mais domínios do `environment.md`. Esta tabela é **estrutural** — não muda por projeto.
...
| 11 | Residência de dados obrigatória | X | X | X | security/compliance, privacy/legal-basis |
...
| 2 | Versionamento (Git provider, branch) |  |  | X | domain/platform-developer-experience |
| 3 | Padrões e convenções (linter, review) |  |  | X | domain/platform-developer-experience |
...
| 2 | Compliance regulatório aplicável | X | X | X | security/compliance, privacy/legal-basis, executive/premises |
...
| 6 | Base legal LGPD/GDPR |  | X | X | privacy/legal-basis |
...
| 2 | Qualidade dos dados (SLA) |  |  | X | domain/data-quality-strategy |
| 3 | Integrações existentes (volume, SLA) |  |  | X | domain/integration-map, domain/integration-data-contracts |
...
| # | Pergunta (resumo) | OP | EX | DR | Region primária |
|---|-------------------|----|----|----|-----------------|
| 1 | Metodologia praticada (Scrum/Kanban) |  | X | X | organization/methodology |
| 2 | Ferramentas de gestão (Jira/ClickUp) |  |  | X | organization/methodology |
...
| # | Pergunta (resumo) | OP | EX | DR | Region primária |
|---|-------------------|----|----|----|-----------------|
| 1 | Composição do time técnico |  | X | X | organization/team-structure |
| 2 | Especialistas disponíveis |  | X | X | organization/team-structure |
| 3 | Riscos de equipe (bus-factor) |  | X | X | risk/risk-matrix |
...
| 2 | Custos de cloud atuais (baseline) |  | X | X | financial/cost-per-component |
...
| 6 | ROI / payback esperado |  |  | X | product/roi, financial/break-even |

**Mínimo D09 para One-Pager:** apenas 2b (estimativa de consumo cloud). Se `financial_model=projeto-paga`, adicionar 4 (budget aprovado). Se `require_roi=true`, 6 passa a `[X][X][X]`.
...
| 2 | Change management |  |  | X | narrative/how-we-got-here |
| 3 | Auditoria (periodicidade, escopo) |  |  | X | security/compliance |
| 4 | SLAs e SLOs corporativos |  | X | X | metrics/slas-and-slos |
| 5 | Propriedade intelectual |  | X | X | narrative/conditions-to-proceed |
...
- [ ] **D01.1** Tipo de infraestrutura
- [ ] **D01.2** Cloud provider
- [ ] **D01.10** Regiões em uso
- [ ] **D01.11** Residência de dados obrigatória
- [ ] **D02.6** Legados críticos sem substituição
- [ ] **D03.7** Débito técnico crítico (pode entrar como premissa "débito existente, escopo a mapear")
- [ ] **D05.2** Compliance regulatório aplicável
- [ ] **D09.2b** Estimativa de consumo cloud do projeto novo

**Se `financial_model=projeto-paga`:** adicionar D09.4 (budget aprovado). **Se `require_roi=true`:** adicionar D09.6 (ROI/payback).

**Excluídos por design (não entram no One-Pager):**
- ~~D08.4~~ Disponibilidade de FTE — One-Pager assume "todos contratados" para simplificar cálculo de esforço.
- ~~D08.5~~ Gaps de skill — tema do Executive.
- ~~D09.2~~ Custo atual de cloud — baseline informativo, não bloqueante.
- ~~D09.4~~ Budget anual — só entra se `financial_model=projeto-paga`.
- ~~D09.6~~ ROI/payback — só entra se `require_roi=true`.
...
- [ ] D09.1, D09.3, D09.5 — modelo de custo, fornecedores, alçadas
- [ ] D10.1, D10.4–6 — catálogo, SLAs, IP, arquitetura ref

## Checklist — Delivery Report (tudo)

Todas as perguntas do `environment.md`. Use o próprio `environment.md` como checklist.

## Como usar durante o Discovery
...
4. Quando o `environment.md` for atualizado, este arquivo **não** precisa mudar estruturalmente — o que muda é o percentual de completude por entregável.
5. O `deliverable-distiller` lê este arquivo como **hint** para decidir granularidade de cada region no OP/EX (ver [!info] no topo).

## Histórico

| Versão | Data | Mudança |
|--------|------|---------|
| 01.00.000 | 2026-04-17 | Criação — origem task #11 do TODO. Generalizado a partir de `projects/patria/kb/question-priority.md`. Bitmap reposicionado como sugestão (não filtro) por ADR-001. |
