...
conditional-on: "financial_model=projeto-paga AND require_roi=true"
---

# Financial Scenarios

Cenários financeiros alternativos gerados quando a receita projetada não cobre o TCO em 3 anos. O solution-architect deve produzir pelo menos 3 cenários que tornem o projeto viável, cada um com TCO revisado, receita projetada, break-even e veredicto. A tabela comparativa permite ao decisor avaliar trade-offs entre cenários.

> [!warning] Region fortemente condicional
> Esta region **só se aplica** quando simultaneamente:
>
> - `financial_model=projeto-paga` — faz sentido comparar cenários de TCO apenas quando o projeto arca com o custo
> - `require_roi=true` — faz sentido contrastar receita vs custo apenas quando há obrigação de justificar ROI
>
> Em modo `fundo-global` ou com `require_roi=false`, **omitir completamente**.
