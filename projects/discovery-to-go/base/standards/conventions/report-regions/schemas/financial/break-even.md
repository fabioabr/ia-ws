...
Identifica o momento em que a receita acumulada (ou economia gerada) supera o investimento acumulado. Este indicador é fundamental para justificar o projeto e definir expectativas de retorno. As premissas subjacentes são explicitadas para permitir revisão crítica.

> [!warning] Region condicional
> Só produzir quando `require_roi=true` no briefing. Por padrão do Discovery-to-Go, a aprovação do investimento acontece **antes** do pipeline — não faz sentido calcular break-even sem um business case de ROI ativo.
