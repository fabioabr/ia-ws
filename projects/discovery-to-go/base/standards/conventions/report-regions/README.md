...
>
> **Subpastas:**
> - `rules/` — 14 subpastas (uma por seção), cada uma com arquivos `.md` definindo as regras de cada region
> - `samples/` — 15 previews HTML (um por seção) para referência visual
...
11 cards · 5 default em todos os projetos
...
### Card REG-PROD-10 — Quick Win

| Campo | Valor |
|-------|-------|
| **ID** | REG-PROD-10 |
| **Path** | `product/quick-win.md` |
| **Fonte** | Bloco #3 (PO) + Solution Architect → 1.3/1.8 |
| **Schema** | Objeto (marco, % projeto, escopo in/out, valor, beneficiários, hipótese, critérios, investimento, riscos mitigados) |
| **Template visual** | Card destacado com pílula "⚡ Quick Win" + mini-timeline interna + split IN/OUT + bloco de valor + tabela de critérios de aceite |
| **Default** | Opcional |
| **Conteúdo** | Entrega intermediária de MVP durante execução do projeto completo — valida hipóteses cedo, gera ROI parcial, reduz risco de projetos longos |
