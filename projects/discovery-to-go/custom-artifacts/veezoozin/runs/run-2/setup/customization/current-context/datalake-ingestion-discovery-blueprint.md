...
| REG-PRIV-01 | Dados pessoais mapeados | Ingestão de datalake quase sempre carrega PII |
| REG-PRIV-02 | Base legal LGPD | Ingestão de datalake quase sempre carrega PII |
| REG-PRIV-03 | DPO e responsabilidades | Ingestão de datalake quase sempre carrega PII |
| REG-PRIV-04 | Política de retenção | Ingestão de datalake quase sempre carrega PII |
...
Regions que o consolidator **pode incluir** conforme contexto do projeto.

| ID | Nome | Quando incluir |
|----|------|----------------|
| REG-PROD-03 | Jornadas de usuário | Quando há personas analíticas com jornadas distintas |
| REG-PROD-06 | Modelo de negócio | Quando datalake é produto monetizado (data-as-a-service) |
| REG-PROD-08 | Roadmap | Quando há faseamento além do MVP |
| REG-PROD-09 | Visão do produto | Quando há horizonte de longo prazo definido |
| REG-ORG-03 | RACI | Quando há múltiplos times ou squads envolvidos |
| REG-ORG-04 | Metodologia | Quando metodologia não está implícita no contexto |
| REG-ORG-05 | On-call e sustentação | Quando há plano de sustentação pós-MVP |
| REG-TECH-04 | Arquitetura de containers | Quando há decisão de containerização relevante (K8s, ECS) |
| REG-TECH-05 | ADRs | Quando há decisões arquiteturais controversas ou não-óbvias |
| REG-TECH-07 | Requisitos não-funcionais | Quando há SLAs rígidos de performance ou disponibilidade |
| REG-SEC-03 | Criptografia | Quando há requisito de BYOK ou criptografia beyond default |
| REG-PRIV-05 | Direito ao esquecimento | Quando datalake é append-only e há exigência de exclusão |
| REG-PRIV-06 | Sub-operadores | Quando há terceiros processando dados pessoais |
| REG-FIN-02 | Break-even analysis | Quando há ROI mensurável e prazo de retorno |
| REG-FIN-03 | Custo por componente | Quando custo de infra é preocupação principal |
| REG-FIN-04 | Projeção de receita | Quando datalake gera receita direta |
| REG-RISK-04 | Análise de viabilidade | Quando há dúvida sobre viabilidade técnica ou regulatória |
| REG-QUAL-03 | Gaps identificados | Quando auditor identifica lacunas relevantes |
| REG-QUAL-04 | Checklist de conclusão | Quando discovery teve múltiplas iterações |
| REG-BACK-02 | User stories de alto nível | Quando épicos precisam de detalhamento adicional |
| REG-BACK-03 | Dependências | Quando há dependências externas críticas |
| REG-BACK-04 | Critérios de Go/No-Go | Quando há gate de aprovação formal |
| REG-METR-02 | KPIs técnicos | Quando há SLAs de latência, uptime ou MTTR |
| REG-METR-03 | SLAs e SLOs | Quando há contratos de nível de serviço formais |
| REG-METR-04 | Targets por fase | Quando roadmap tem metas distintas por fase |
| REG-NARR-02 | Condições para prosseguir | Quando há pré-requisitos bloqueantes |
| REG-NARR-03 | Assinaturas de aprovação | Quando há gate formal de sign-off |
| REG-PESQ-01 | Relatório de entrevistas | Quando há entrevistas com stakeholders de dados |
| REG-PESQ-02 | Citações representativas | Quando há quotes relevantes de entrevistas |
| REG-PESQ-03 | Mapa de oportunidades | Quando há múltiplas oportunidades de uso dos dados |
| REG-PESQ-04 | Dados quantitativos | Quando há métricas de volume ou impacto quantificáveis |
| REG-PESQ-05 | Source tag summary | Quando se deseja rastrear origem das informações |

### Domain-specific

Regions exclusivas do context-template `datalake-ingestion`.

| ID | Nome | Descrição | Template visual |
|----|------|-----------|-----------------|
| REG-DOM-DATA-01 | Medallion architecture | Diagrama Bronze → Silver → Gold com transformações, retenção, particionamento | Diagram full-width |
| REG-DOM-DATA-02 | Data quality strategy | Framework de qualidade, testes por camada, freshness SLA | Table com status |
